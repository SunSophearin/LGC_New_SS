        ...
        />
          <div class="w-full">
            <ul class="flex w-full flex-col mt-10 font-sixtyfour">
              <li
                class="w-full flex p-4 border-b-2 hover:bg-blue-500 hover:text-white cursor-pointer"
                v-for="item in listMenu"
                :key="item.id"
                @click="handlenameSub(item.name)"
              >
                <NuxtLink class="w-full" :to="`/products/${item.id}/catagory`" @click="isOpen = false">{{
                  item.name
                }}</NuxtLink>
                <UIcon name="heroicons:chevron-right-20-solid" class="w-5 h-5" />
              </li>
            </ul>
          </div>
      </div>
